...
export default function Navbar() {
  ...
  const navTheme = isCareerPage
      ? "bg-black/70 text-white"
      : "bg-white/70 text-black";

  const buttonTheme = isCareerPage
      ? "bg-white text-black"
      : "bg-black text-white"

  const mobileMenuTheme = isCareerPage
    ...
    <nav
      ...
      <div className="relative flex justify-between items-center px-6 py-4">
        ...
        <div className="md:hidden flex items-center text-xs gap-2 z-30">
           <HashLink smooth to= {isCareerPage ? "#career-connect" :"/#connect"}onClick={handleLinkClick} >
        ...
        <div className="hidden md:flex items-center gap-8 font-medium z-30">
          <HashLink smooth to="/#hero" className="hover:text-red-500 transition">Home</HashLink>
          ...
          <HashLink smooth to="/#connect">
            <button className={`${buttonTheme} px-4 py-2 rounded hover:opacity-90 transition duration-300`}>
              Contact Us
            </button>
          </HashLink>
        </div>
      ...
      {/* Mobile Menu */}
      {isOpen && (
       <div className={`md:hidden px-6 pb-4 pt-2 space-y-4 flex flex-col font-medium z-20 relative backdrop-blur-xl rounded-b-xl shadow ${mobileMenuTheme}`}>
          <HashLink smooth to="/#hero" className="hover:text-red-500 transition" onClick={handleLinkClick}>Home</HashLink>
